...
FIGURE_HINT_RE = re.compile(
    # 編號標示：圖一、圖(一)、圖二、圖(2)、Figure 1
    r'圖[一二三四五六七八九十]+'
    r'|圖\s*[(（]\s*[一二三四五六七八九十0-9]+\s*[)）]'
    r'|圖\s*\d+'
    # 方位／指代
    r'|如[右左上下]?圖|[下右左上]圖|附圖|見圖|參考圖|根據圖|圖中|圖示'
    # 統計圖類
    r'|示意圖|盒狀圖|盒形圖|箱型圖|箱形圖|統計圖|長條圖|柱狀圖'
    r'|圓餅圖|圓形圖|折線圖|分布圖|散布圖|散佈圖'
    # 結構圖類
    r'|流程圖|電路圖|地形圖|地圖|平面圖|立體圖|展開圖|示力圖|作圖'
    # 數學常見：坐標／座標 + 平面 / 圖形
    r'|[坐座]標平面'
)
